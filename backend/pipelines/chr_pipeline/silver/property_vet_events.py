...
def create_property_vet_events_table(con: ibis.BaseBackend, ejendom_vet_raw: ibis.Table | None, lookup_tables: dict[str, ibis.Table | None], silver_dir: Path) -> ibis.Table | None:
    ...
    try:
        ...
        vet_events = vet_events.mutate(
            event_id=ibis.uuid(),
            chr_number=ibis.coalesce(vet_events.chr_number_raw.pipe(_sanitize_string).cast(dt.int64), ibis.null().cast(dt.int64)),
            has_vet_problems=(vet_events.has_vet_problems_raw.cast(dt.string).pipe(_sanitize_string).lower() == 'ja'),
            species_code=ibis.coalesce(vet_events.species_code.cast(dt.string).pipe(_sanitize_string).cast(dt.int32), ibis.null().cast(dt.int32)),
            species_name=_sanitize_string(vet_events.species_name),
            disease_code=_sanitize_string(vet_events.disease_code),
            disease_name=_sanitize_string(vet_events.disease_name),
            vet_status_code=_sanitize_string(vet_events.vet_status_code),
            vet_status_name=_sanitize_string(vet_events.vet_status_name),
            disease_level_code=_sanitize_string(vet_events.disease_level_code),
            disease_level_name=_sanitize_string(vet_events.disease_level_name.cast(dt.string)),
            vet_status_date=ibis.coalesce(vet_events.vet_status_date.cast(dt.string).pipe(_sanitize_string).cast(dt.date), ibis.null().cast(dt.date)),
            remark=_sanitize_string(vet_events.remark)
        )
        ...
        final_cols = [
            ...
            'disease_level_code', 'disease_level_name', 
            'vet_status_code', 'vet_status_name',
            'vet_status_date', 
            'has_vet_problems', 'remark' 
        ]
